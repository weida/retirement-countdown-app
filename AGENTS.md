...
`capacitor.config.json` sets `appId: com.codex.retirementcountdown`, `webDir: dist`. The `android/` and `ios/` directories are gitignored — they're regenerated by `npx cap add <platform>` (the CI workflow does this on every run).
